...
const modalCheckoutName = $("#cf-checkout-name");
const modalCheckoutPayment = $("#cf-checkout-payment");
...
function getUtmSuffix() {
  try {
    ...
    const bits = [source && `source=${source}`, medium && `medium=${medium}`, campaign && `camp=${campaign}`]
      .filter(Boolean)
      .join(" ");
    return `\n[origem: ${bits}]`;
  } catch {
...
$("#checkout-btn").addEventListener("click", () => {
  ...
  modalOrder.textContent = `${lines.join("\n")}\nTotal: ${totalText}`;
  modalCheckoutName.value = "";
  modalCheckoutPayment.value = "Pix";
...
function updateModalWhatsApp() {
  const name = modalCheckoutName.value.trim();
  const payment = modalCheckoutPayment.value || "Pix";
  const base = modalOrder.textContent;
  const withName = `Olá, Ju! Meu nome é ${name || "[seu nome]"}. Quero finalizar meu pedido:\n${base}\nPagamento: ${payment}${getUtmSuffix()}`;
  modalWhatsApp.href = `https://wa.me/5521996851605?text=${encodeURIComponent(withName)}`;
}

[modalCheckoutName, modalCheckoutPayment].forEach((el) => {
  el.addEventListener("input", updateModalWhatsApp);
  el.addEventListener("change", updateModalWhatsApp);
});

modalWhatsApp.addEventListener("click", (event) => {
  if (!modalCheckoutName.value.trim()) {
    event.preventDefault();
    modalCheckoutName.focus();
    modalCheckoutName.classList.add("form-input-error");
    return;
  }
  modalCheckoutName.classList.remove("form-input-error");
});
